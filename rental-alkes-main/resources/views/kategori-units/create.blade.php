<x-app-layout>
    <x-slot name="header">
        <h2 style="font-size: 1.375rem; font-weight: 800; color: #0f172a; margin: 0; letter-spacing: -0.025em;">
            Tambah Kategori Unit Alkes
        </h2>
        <p style="font-size: 0.813rem; color: #64748b; margin: 3px 0 0 0; font-weight: 500;">Buat kelompok jenis alat kesehatan baru ke dalam sistem.</p>
    </x-slot>
    ...
    <div style="max-width: 700px; margin: 0 auto; background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02); overflow: hidden;">
        ...
        <form action="{{ route('kategori-units.store') }}" method="POST" style="padding: 24px;">
            @csrf

            <div style="margin-bottom: 20px;">
                <label style="display: block; font-size: 0.875rem; font-weight: 700; color: #475569; margin-bottom: 8px;">Nama Kategori</label>
                <input type="text" name="nama_kategori" value="{{ old('nama_kategori') }}" placeholder="Contoh: Alat Terapi atau Cek Darah" required style="width: 100%; padding: 10px 14px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem; color: #334155; outline: none;">
            ...
            <div style="margin-bottom: 24px;">
                ...
                <textarea name="deskripsi" rows="3" placeholder="Keterangan singkat mengenai kategori alat ini..." style="width: 100%; padding: 10px 14px; border-radius: 8px; border: 1px solid #cbd5e1; font-size: 0.875rem; color: #334155; outline: none; resize: vertical;">{{ old('deskripsi') }}</textarea>
