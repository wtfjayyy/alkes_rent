<x-app-layout>
    <x-slot name="header">
        ...
        <p style="font-size: 0.813rem; color: #64748b; margin: 3px 0 0 0; font-weight: 500;">Perbarui data antrean atau pesanan sewa alat kesehatan.</p>
    </x-slot>
            ...
            <div style="background: #ffffff; padding: 32px; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02);">
                ...
                <form action="{{ route('bookings.update', $booking->id) }}" method="POST">
                        ...
                        <select name="customer_id" id="customer_id" style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.875rem; background: #fff;" required>
                            ...
                            @isset($customers)
                                @foreach($customers as $customer)
                                    <option value="{{ $customer->id }}" data-phone="{{ $customer->no_hp ?? $customer->telepon ?? $customer->whatsapp ?? $customer->no_whatsapp ?? '' }}" {{ old('customer_id', $booking->customer_id) == $customer->id ? 'selected' : '' }}>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <input type="text" name="no_whatsapp" id="no_whatsapp" value="{{ old('no_whatsapp', $booking->no_whatsapp) }}" placeholder="Contoh: 081234567890" style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.875rem;" required>
                    </div>
                        ...
                        <select name="unit_id" id="unit_id" style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.875rem; background: #fff;" required>
                            ...
                            @isset($units)
                                @foreach($units as $unit)
                                    <option value="{{ $unit->id }}" {{ old('unit_id', $booking->unit_id) == $unit->id ? 'selected' : '' }}>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <input type="date" name="estimasi_tgl_mulai" id="estimasi_tgl_mulai" value="{{ old('estimasi_tgl_mulai', $booking->estimasi_tgl_mulai) }}" style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.875rem;" required>
                    </div>
                    ...
                    <div style="margin-bottom: 24px;">
                        ...
                        <textarea name="catatan" id="catatan" rows="3" placeholder="Tuliskan catatan atau keperluan khusus..." style="width: 100%; padding: 10px 12px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 0.875rem; resize: vertical;">{{ old('catatan', $booking->catatan) }}</textarea>
                    </div>
                    ...
                    <div style="display: flex; justify-content: flex-end; gap: 12px;">
                        ...
                        <button type="submit" style="padding: 10px 20px; background-color: #059669; color: white; border: none; border-radius: 8px; font-weight: 600; cursor: pointer; font-size: 0.875rem; box-shadow: 0 4px 6px rgba(5,150,105,0.2);">Perbarui Booking</button>
                    </div>
                </form>
