<x-app-layout>
    <x-slot name="header">
        <h2 style="font-size: 1.375rem; font-weight: 800; color: #0f172a; margin: 0; letter-spacing: -0.025em;">
            Manajemen Kategori Unit
        </h2>
        <p style="font-size: 0.813rem; color: #64748b; margin: 3px 0 0 0; font-weight: 500;">Kelola pengelompokan jenis alat kesehatan.</p>
    </x-slot>

    <!-- Posisinya sekarang beneran ada di kanan bawah header, sama kayak Unit -->
    <div style="display: flex; justify-content: flex-end; margin-bottom: 24px; gap: 12px;">
        
        <!-- Tombol Kembali (Back) -->
        <a href="{{ route('units.index') }}" style="background: #ffffff; color: #475569; padding: 10px 20px; border-radius: 10px; font-weight: 700; font-size: 0.875rem; text-decoration: none; border: 1px solid #cbd5e1; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02); display: inline-flex; align-items: center; gap: 6px; transition: all 0.2s;">
            <span>&larr;</span> Kembali ke Daftar Unit
        </a>

        <!-- Tombol Tambah Kategori -->
        <a href="{{ route('categories.create') }}" style="background: #059669; color: #ffffff; padding: 10px 20px; border-radius: 10px; font-weight: 700; font-size: 0.875rem; text-decoration: none; box-shadow: 0 4px 6px rgba(5, 150, 105, 0.2); display: inline-flex; align-items: center; gap: 8px;">
    ...
    <!-- Card tabel lebar penuh dengan border radius 16px -->
    <div style="background: #ffffff; border-radius: 16px; border: 1px solid #e2e8f0; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02); overflow: hidden;">
        <div style="padding: 20px 24px; border-bottom: 1px solid #e2e8f0; font-size: 0.875rem; font-weight: 700; color: #334155; text-transform: uppercase; letter-spacing: 0.05em;">
            Daftar Kategori Alat Kesehatan
        </div>
            ...
            <table style="width: 100%; border-collapse: collapse; text-align: left;">
                ...
                <tbody style="font-size: 0.875rem; color: #334155;">
                    @isset($categories)
                            ...
                            <tr style="border-bottom: 1px solid #f1f5f9;">
                                ...
                                <td style="padding: 16px 24px; text-align: center;">
                                    <div style="display: flex; gap: 8px; justify-content: center;">
                                        <a href="{{ route('categories.edit', $cat->id) }}" style="background: #eff6ff; color: #2563eb; padding: 6px 12px; border-radius: 6px; font-weight: 600; font-size: 0.75rem; text-decoration: none;">Edit</a>
                                        
                                        <form action="{{ route('categories.destroy', $cat->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus kategori ini?');" style="margin: 0;">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" style="padding: 32px; text-align: center; color: #64748b;">Belum ada data kategori yang terdaftar.</td>
                            </tr>
                        @endforelse
                    @endisset
                </tbody>
